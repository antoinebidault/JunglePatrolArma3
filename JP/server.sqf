...
CIVILIAN_KILLED = { 
	...
	[GROUP_PLAYERS,-10,false,_killer] remoteExec ["JP_fnc_updateScore",2];
};
...
CIVIL_HEALED = { 
	[GROUP_PLAYERS,10,false,(leader GROUP_PLAYERS)]  remoteExec ["JP_fnc_updateScore",2];
 };

//If civil is captured
 CIVIL_CAPTURED = { 
	//[GROUP_PLAYERS,-5] call JP_fnc_updateScore;
 };

// If player is killed
 PLAYER_KIA = { 
	[GROUP_PLAYERS,-2,false,(leader GROUP_PLAYERS)] remoteExec ["JP_fnc_updateScore",2];
 };
...
CLUSTERS = [];
[] spawn {
	CLUSTERS = [GAME_ZONE] call JP_fnc_getClusters;
};
...
[] spawn {
	...
	_officerPos = _spawnpos;
	{
		if (_x isKindOf "ReammoBox_F") then {
			_x remoteExec ["JP_fnc_spawncrate",0,true];
		};
	} foreach _compoObjs; 
	...
	for "_j" from 1 to 6 do {
		...
		if (_j == 1) then {
			_unit remoteExec ["JP_fnc_addActionInstructor",0, true];
...
// [] spawn JP_fnc_spawnSecondaryObjective; 
[] spawn JP_fnc_spawnMainObjective;
[] call JP_fnc_refreshMarkerStats; // Refresh marker stats
[] spawn JP_fnc_spawnSheep;
