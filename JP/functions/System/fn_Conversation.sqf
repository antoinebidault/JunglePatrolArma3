...
_filePath = "JP\voices\" + _file + "\CfgSentences.bikb";
...

{
	...
	_sentenceText = getText(missionConfigFile >> "CfgSentences" >> "JP" >> _file >> "Sentences" >> _text >> "text");
	[_talker, _sentenceText] spawn JP_fnc_showSubtitle;

	waitUntil {
		_talker kbWasSaid [_spectator, _topic,_text, 3];
	};

	[] call JP_fnc_hideSubtitle;
