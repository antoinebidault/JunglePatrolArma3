...
[_player] spawn JP_fnc_surrenderSystem;
...
if ((leader GROUP_PLAYERS) == _player) then {
	...
	sleep .3;
	// _player call JP_fnc_actionCamp;
	// _player call JP_fnc_addSupportUi;
};

JP_fnc_handleRespawnBase = {
	params["_unit"];
    resetCamShake;
...
//Respawn handling
// Singleplayer
JP_fnc_handleRespawnSingleplayer =
{
	params["_unit"];
	...
	[_unit] call JP_fnc_handleRespawnBase;
...
if (RESPAWN_ENABLED) then{
	...
	if (isMultiplayer) then {
	    ...
	    _player addMPEventHandler ["MPRespawn", {
			...
			[_unit] spawn JP_fnc_handleRespawnBase;
		}];
		...
		_player addEventHandler["HandleDamage",{
			...
			if (_damage >= .9 && lifeState _unit != "INCAPACITATED" )then{
				...
				_damage = .9;
				_unit setDamage .9;
				[_unit] spawn JP_fnc_handleRespawnSinglePlayer;
			} else {
				if (lifeState _unit == "INCAPACITATED")then{
					_damage = .9;
					_unit setDamage .9;
