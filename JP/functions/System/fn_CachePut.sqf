...
{
    _type = _x getVariable["JP_Type",""];
    _cacheDisabled = _x getVariable["JP_disable_cache",false];
    _elt = _x;

    // Disable cache if unit is very close to a player
    if (_x isKindOf "LandVehicle" && { vehicle _x == _elt } count ([] call JP_fnc_allPlayers) > 0) then {
        _cacheDisabled = true;
    }; 
