...
if (count _buildings == 0) exitWith {[_grp,_radius] call JP_fnc_simplePatrol;};

//private _buildings = [getPos _unit,_radius] call JP_fnc_findBuildings;
...
scopeName "main";
while { alive _unit && !(_unit getVariable ["JP_disable_patrol",false]) }do{
    ...
    while{_i2 < (count _bPoss)}do{
        ...
        if (_rd == 0) then {
            ...
            if (_unit getVariable ["JP_disable_patrol",false] || !alive _unit)then{breakTo "main";};
            ...
            if (_rd==1) then{
                ...
                if (_unit getVariable["JP_Suspect",false] && !stopped _unit)then{
                    _unit stop true;
                ...
                [_unit,_meetPoint] call JP_fnc_gotomeeting;
                if (_unit getVariable ["JP_disable_patrol",false] || !alive _unit)then{breakTo "main";};
                sleep 20;
            };
