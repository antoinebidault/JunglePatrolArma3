...
JP_fnc_dropAction = {
	...
	_injured setVariable ["JP_unit_dragged", false, true];
	...
	_injured call JP_fnc_addActionHeal;
};


if (isNull _injured) exitWith {};
if (!alive _injured) exitWith {};
if (!(_injured getVariable ["JP_unit_injured", false])) exitWith {};

_addAction = false;
_dropActionId = 0;
_injured setVariable ["JP_unit_dragged", true, true];

while {_injured getVariable ["JP_unit_dragged", false]} do {
	if (!_addAction) then {
		...
		_dropActionId = _unit addAction [localize "STR_JP_carry_drop", JP_fnc_dropAction,_injured, 0, false, true];
	...
	if (vehicle _unit != _unit) then {
		...
		[cursorTarget, _unit, _unconscious, _injured] call JP_fnc_dropAction;
	};
