...
if (!(player getVariable ["JP_buildItemGrabbed",false])) then {
	...
	if (_closestPlayerDist > 4) then {
			...
			_actionID = _caller addAction [
				...
				'[_this select 3, _this select 1, _this select 2,true] call JP_fnc_placeObject;',
				_object
			];

			_caller setVariable["JP_build_action",_actionID ,true];
			_caller setVariable ["JP_buildItemGrabbed", true, true];
			_object setVariable ["JP_buildItemGrabbed", true, true];
