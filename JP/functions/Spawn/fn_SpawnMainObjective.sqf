...
ENEMY_COMMANDER execVM "JP\loadout\loadout-commander.sqf";
...
ENEMY_COMMANDER addEventHandler ["FiredNear",{
    ...
    if (group _gunner == GROUP_PLAYERS && _distance < 50)then{
        [_gunner,"Shit ! this asshole is fleeing.",false]  remoteExec ["JP_fnc_talk"];
        ...
        [_gunner,_commander] spawn {
            ...
            _commander call JP_fnc_deleteMarker;
            deleteVehicle _commander;
            [_gunner ,"He has definitely left the area... Mission compromised. Maybe we would catch him later..." ,true ] remoteExec ["JP_fnc_talk"];
            [] spawn JP_fnc_spawnMainObjective;
        };
    };
}];
...
ENEMY_COMMANDER addMPEventHandler ["MPKilled",{
    ...
    if (group _killer == GROUP_PLAYERS)then{
        [_killer,{
            ...
            [_this, format["HQ ! This is %1, the enemy commander is KIA ! Out.",name _this],true] remoteExec ["JP_fnc_talk"];
            [HQ, "Good job everyone ! We're sending you a chopper to the extraction point !",true] remoteExec ["JP_fnc_talk"];
...
while {alive (leader _grp) && leader _grp == ENEMY_COMMANDER}do{
    ...
    _trig setTriggerStatements[
        "this",
        " [thisList select 0] spawn JP_fnc_foundCommander;",
