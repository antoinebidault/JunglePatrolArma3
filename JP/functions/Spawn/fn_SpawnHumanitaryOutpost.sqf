...
for "_xc" from 1 to _nb  do {
   ...
   //   _unitName = ALLIED_LIST_UNITS call BIS_fnc_selectRandom;
    //  _unit = _grp createUnit [_unitName, _posSelected,[],1,"NONE"];
      ...
      _unit = [_grp,_posSelected,false] call JP_fnc_spawnhumanitar;
      _unit setVariable["JP_Type","humanitar"];
      _units pushBack _unit;

      //Si c'est une patrouille
       // spawn JP_fnc_humanitarPatrol;
			[_grp,"JP_fnc_humanitarPatrol", [_grp,_radius,_meetingPointPosition,_buildings,_pos]] call JP_fnc_patrolDistributeToHC;
};
