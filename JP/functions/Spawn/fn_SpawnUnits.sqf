...
//List positions;
private _posResult = [];
private _posResult = [_pos,_radius,_buildings,floor(_population/2)] call JP_fnc_spawnPosition;
...
for "_xc" from 1 to _nbenemies  do {
   ...
   if (count _posSelects > 0)then{
    ...
    for "_xc" from 1 to _nbUnit do {
      ...
      _enemy setVariable["JP_Type","enemy"];
      _enemy setDir random 360;
      _units pushBack _enemy;
    };

   // [_grp,_radius,_meetingPointPosition,_buildings] spawn JP_fnc_enemyCompoundPatrol;
...
for "_xc" from 1 to _population do {
    ...
    if (count _posSelects > 0)then{
      ...
      _posSelects = _posSelects - [_posSelected];
        ...
        if (count _buildings > 0) then {
          _randomBuilding = _buildings call BIS_fnc_selectRandom;
    ...
    if (count _posSelected > 0) then {
     ...
     /* if (_xc == 1 && _population >= 1 && count _buildings > 0) then {
        _civ = [_grp,_posSelected,_chief,false] call JP_fnc_spawnCivil;
        [_civ,_radius] call JP_fnc_localChief;
        ...
        if (_xc == 2 && _population > 10)then{
          _civ = [_grp,_posSelected,_chief,false,"C_Marshal_F"] call JP_fnc_spawnCivil;
         removeAllWeapons _civ;
          _civ call JP_fnc_medic;
          _units pushBack _civ;
        }else{
          if (_xc == 3 && _compoundState == "resistance") then {
              _advisor = [_grp, _posSelected, false] call JP_fnc_spawnfriendly;
              _units pushback _advisor;
          } else{
            _civ = [_grp,_posSelected,_chief,if (_compoundState == "resistance") then {false}else{true},nil,_compoundScore] call JP_fnc_spawnCivil;
            if (_compoundState == "massacred" || _compoundState == "humanitary") then {
              
              _civ setDamage ([.9,.85,.5,.7,.3] call BIS_fnc_selectRandom);
              if (damage _civ <= .6) then {
		            _civ call JP_fnc_addActionHeal;
              };
           ...
           // [_grp,_radius,_meetingPointPosition,_buildings] spawn JP_fnc_civilianCompoundPatrol;
            [_grp,"JP_fnc_civilianCompoundPatrol",[_grp,_radius,_meetingPointPosition,_buildings]] call JP_fnc_patrolDistributeToHC;

            // If "resistance"
            if (_compoundState == "resistance") then {
              [_civ, SIDE_FRIENDLY] remoteExec ["JP_fnc_badGuyLoadOut",owner _civ];
              [_civ] remoteexec ["JP_fnc_addCivilianAction",0];
            };
