...
if((JP_SCORE - _price) >= 0)then {
	
	//Fermeture plus MAJ score 
	closeDialog 0;
	
	[GROUP_PLAYERS,-_price] remoteExec ["JP_fnc_updateScore",2];   
	
	if (_type=="UAV")then{
		[HQ,"An UAV is moving toward your position",true] remoteExec ["JP_fnc_talk"];
		...
		if (_type=="vehicle") then {
			[HQ,"Support provided",true] remoteExec ["JP_fnc_talk"];
			[_leader,{
