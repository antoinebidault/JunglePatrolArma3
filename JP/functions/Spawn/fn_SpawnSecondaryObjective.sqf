...
JP_fnc_spawnOfficerSimple = {
    ...
    if (count _roadConnectedTo == 0) exitWith { hint "restart"; [] call JP_fnc_spawnOfficerSimple; };
...
for "_i" from 1 to NUMBER_OFFICERS  do {
    _officer = [] call JP_fnc_spawnOfficerSimple;
...
[HQ,format[localize "STR_JP_voices_HQ_goodJob", NUMBER_OFFICERS], true] remoteExec ["JP_fnc_talk"];
[HQ,format[localize "STR_JP_voices_HQ_nextStep" , name ENEMY_COMMANDER], true] remoteExec ["JP_fnc_talk"];
