...
if (_unitWithTask getVariable["JP_IsIntelRevealed",false])then{
    _unitWithTask setVariable["JP_IsIntelRevealed",false];
    deleteMarker(_unitWithTask getVariable["JP_MarkerIntel",""]);
    [_unitWithTask getVariable["JP_Task",""], "FAILED",true] remoteExec ["BIS_fnc_taskSetState",GROUP_PLAYERS];

    // Spawn task successful on each client
    [[_unitWithTask getVariable["JP_Task",""]],{
