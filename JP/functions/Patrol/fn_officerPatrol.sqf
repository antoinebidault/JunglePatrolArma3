...
while {sleep 5;  alive _unit && alive _truck && alive _driver}do{
    ...
    _compound = [_curPos, true, "any"] call JP_fnc_findNearestMarker;
...
*/
    ...
    if ((_compound select 13) > 70) then {
        [_compound,"massacred"] spawn JP_fnc_setCompoundState;
        [_compound,15,100] spawn JP_fnc_setCompoundSupport;
    } else {
        [_compound,"bastion"] spawn JP_fnc_setCompoundState;
        [_compound,-30,100] spawn JP_fnc_setCompoundSupport;
    };*/
