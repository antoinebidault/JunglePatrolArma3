...
//  if (!([GROUP_PLAYERS,_shopPrice] call JP_fnc_afford)) exitWith {hint "Can't buy this item"; };
...
if (!isNil '_initField') then{
	_obj setVariable["JP_initField",_initField,true];
};
_obj setVariable["JP_price",_shopPrice,true];
[_obj, player, [0,_vecRadius + 1.5,0.02], _shopDir] call JP_fnc_grabObject; 