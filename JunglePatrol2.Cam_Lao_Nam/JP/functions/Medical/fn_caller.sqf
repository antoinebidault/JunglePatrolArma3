...
if (MEDEVAC_State == "menu") then {
	...
	MEDEVAC_MENU_LASTID = [_unit, localize "STR_JP_fnCaller_medevacMenu"] call BIS_fnc_addCommMenuItem;
};

if (MEDEVAC_State == "inbound") then{
	MEDEVAC_action = _unit addAction [localize "STR_JP_fnCaller_abortMedevac", { 
