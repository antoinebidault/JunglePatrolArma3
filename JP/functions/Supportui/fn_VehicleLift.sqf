...
if (_type == "ammo")  then {
	_cargo call JP_fnc_spawnCrate;
} else {
	if (_type == "buildingKit") then {
		[_cargo,["<t color='#cd8700'>Recruit a guard</t>", { [] call JP_fnc_recruitDialog; },nil,2.5,false,true,"","true",20,false,""]] remoteExec ["addAction",0,true];
		[_cargo,["<t color='#cd8700'>Buy objects</t>", { [] call JP_fnc_buildingDialog; },nil,2.5,false,true,"","true",20,false,""]] remoteExec ["addAction",0,true];
	};
};
...
if (!_success) exitWith { 
    [GROUP_PLAYERS,150] remoteExec ["JP_fnc_updateScore",2];   
	{deleteVehicle _x} foreach crew _chopper; 
	deleteVehicle _chopper; 
	deleteVehicle _cargo;
	[_pos,_dist,_type,"B_Heli_Transport_03_F"] call JP_fnc_vehicleLift;
};
