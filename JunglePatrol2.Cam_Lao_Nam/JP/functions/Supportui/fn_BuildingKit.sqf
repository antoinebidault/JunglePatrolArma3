...
onMapSingleClick {
	...
	if ({_pos inArea _x } count MARKER_WHITE_LIST > 0) then {
		openMap false;
		[HQ,localize "STR_JP_support_ImpossibleToDrop"] spawn JP_fnc_talk;
	} else {
		if (count BASE_NAMES == 0) then {
			openMap false;
			[HQ,localize "STR_JP_support_TooMany" ] spawn JP_fnc_talk;
		} else {
...
[getMarkerPos Base_marker,2500, "buildingKit"] spawn JP_fnc_vehicleLift;
