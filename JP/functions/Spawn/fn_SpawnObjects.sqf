...
for "_xc" from 1 to _nbCompos do {
  ...
  /*  if (random 100 > 70) then {
        ...
        _turrets = nearestObjects [ _nicePos, [ "LandVehicle" ], 20 ];
         if (count _turrets > 0 ) then {
            ...
            if (_side == SIDE_ENEMY) then {
                _unit = [createGroup _side, _nicePos, false] call JP_fnc_spawnenemy;
            } else{
                _unit = [createGroup _side, _nicePos, false] call JP_fnc_spawnfriendly;
            };
            [_newObjs select 0, "ColorPink"] call JP_fnc_addMarker;
        ...
        _newObjs = [_nicePos,  random 360, _curr] call BIS_fnc_ObjectsMapper;
         [_newObjs select 0, "ColorWhite"] call JP_fnc_addMarker;
        _objs = _objs + _newObjs;
