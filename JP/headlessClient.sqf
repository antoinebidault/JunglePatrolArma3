...
JP_fnc_sendToHC = {
  ...
  _sortedHCs = [HCs, [], {(_x call JP_fnc_countUnitInHC)}, "ASCEND"] call BIS_fnc_sortBy;
  ...
  if (count HCs > 0) then {
    for "_i" from 0 to (count HCs - 1) do{ 
      ...
      if (isNull _hc) then {
        ...
        diag_log format ["[HC] %1 AI groups currently on %2", _hc call JP_fnc_countUnitInHC, str _hc]; 
      };
    };
  };
