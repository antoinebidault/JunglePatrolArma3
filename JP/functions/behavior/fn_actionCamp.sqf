...
*/
 ...
 _this addAction [format["<t color='#00FF00'>%1 (1 hour)</t>", localize "STR_JP_actionCamp_setUpAction"], {
    params["_unit","_asker","_action"];

    if((_unit findNearestEnemy _unit) distance _unit < 100) exitWith {
        hint localize "STR_JP_actionCamp_error" ;
    };
    ...
    if(([getPos _unit, 1, getDir _unit ] call BIS_fnc_relPos) isFlatEmpty  [2, -1, 0.3, 2, 0, false, _unit] isEqualTo [] 
        ...
        hint localize "STR_JP_actionCamp_error2" ; 
    };
    ...
    [_unit,localize "STR_JP_voices_teamLeader_campSetUp"] remoteExec ["JP_fnc_talk",0, false];

    hint localize "STR_JP_actionCamp_hint";
        
    // Pack up camp
    (CAMP_OBJS select 0) addAction [format["<t color='#00FF00'>%1</t>",localize "STR_JP_actionCamp_packUpAction"], {
        params["_unit","_asker","_action"];
        _unit call JP_fnc_actioncamp;
        ...
        sleep 3;
        titleCut ["", "BLACK IN", 3];
        [_asker,localize "STR_JP_voices_teamLeader_campPackedUp" ] remoteExec ["JP_fnc_talk",0, false];
            sleep 3;
        disableUserInput false;
    },nil,4,true,true, ""];
    ...
    //Rest animations
    (CAMP_OBJS select 18) remoteExec ["JP_fnc_actionrest",0, true];
